...
public class Sorts {
    ...
    public static <T extends Comparable<? super T>> void MaxHeapify(T arr[], int arrLength, int i)
    {
        int largest = i;
        int left = 2*i + 1;
        int right = 2*i + 2;
        if (left < arrLength && arr[left].compareTo(arr[largest]) > 0)
            largest = left;
        if (right < arrLength && arr[right].compareTo(arr[largest]) > 0)
            largest = right;
        if (largest != i)
        {
            T swap = arr[i];
